import{g as c,D as d,b as u,a as m,t as b,d as $}from"./Data-BJxmCOTU.js";function h(e){const s=[];e.streaming.forEach(a=>{const n=`<a href="${a.url}" target="_blank"><span>${a.name}</span></a>`;s.push(n)});const l=[];e.studios.forEach(a=>{const n=a.name;l.push(n)});const r=[];e.themes.forEach(a=>{const n=a.name;r.push(n)});const t=[];e.producers.forEach(a=>{const n=a.name;t.push(n)});const o=[];e.demographics.forEach(a=>{const n=a.name;o.push(n)});const p=e.trailer&&e.trailer.embed_url?`<div class="youtube-trailer">
